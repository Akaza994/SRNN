...
def threshold_single_variable(sess, var, threshold):
    var_value = sess.run(var)  # var_value is the np array with the current value of var.
    var_value[abs(var_value) < threshold] = 0  # this thresholds small values
    # Tensorflow value assignment is very stupid..
    var_value_placeholder = tf.placeholder(dtype=tf.float32, shape=var_value.shape)
    sess.run(tf.assign(var, var_value_placeholder), feed_dict={var_value_placeholder : var_value})
...
class SRNNBase(): 
    ...
    def load_model(self, sess, model_path, global_step=None):
     ...
     # Xiyang Add
    def threshold_rnn_weights(self, sess, threshold=0.0001):
        # This extracts all trainable variables for all RNN cells.
        rnn_cell_vars = [var for var in tf.trainable_variables() if 'cell' in var.name] 
        # This applys hard thresholding to all of the weights in each cell.
        for var in rnn_cell_vars:
            threshold_single_variable(sess, var, threshold)   
    ...
    def fit_model(
        ...
        for t in self.types: 
            ...
            #l1_regularizer = tf.contrib.layers.l1_regularizer(scale=0.00000005, scope=None)
            ...
            #la: NOT WORKING
            #weights=[tf.multiply(2*tf.abs(v),tf.reciprocal(1+tf.abs(v))) for v in tv if len(v.shape)>1]
            #l1_regularizer = tf.contrib.layers.l1_regularizer(scale=0.00000005, scope=None)
            #regularization_cost = tf.contrib.layers.apply_regularization(l1_regularizer, weights)
        ...
        threshold = 0.1
        self.threshold_rnn_weights(sess, threshold)
        ...
        for k,v in zip(variables_names, values):
            if len(v.shape)>1:
                ...
                #print tf.count_nonzero(tf.less(v,0.001+tf.zeros(v.shape))).eval()
